(() => {
    ...
    function initSliders() {
        if (document.querySelector(".two__slider")) new core(".two__slider", {
            ...
            navigation: {
                prevEl: ".two__swiper-button-prev",
                nextEl: ".two__swiper-button-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
        ...
        if (document.querySelector(".helicopter__slider")) {
            const helicopterSlider = new core(".helicopter__slider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 1,
                ...
                slidesPerGroup: 1,
                pagination: {
                    el: ".helicopter__swiper-pagination-number",
                    clickable: true,
                    type: "fraction"
                },
                navigation: {
                    prevEl: ".helicopter__swiper-button-prev",
                    nextEl: ".helicopter__swiper-button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            const swiperHelicopteMenu = document.querySelector(".swiper-helicopter-menu");
            let items = swiperHelicopteMenu.children;
            swiperHelicopteMenu.addEventListener("click", (() => {
                for (let i = 0; i < items.length; i++) items[i].classList.remove("active");
                let slideItem = event.target;
                slideItem.classList.add("active");
                let slideIndex = event.target.dataset.index;
                helicopterSlider.slideTo(slideIndex);
            }));
            helicopterSlider.on("activeIndexChange", (function() {
                for (let i = 0; i < items.length; i++) {
                    items[i].classList.remove("active");
                    if (items[i].dataset.index == helicopterSlider.activeIndex) items[i].classList.add("active");
                }
            }));
        }
    ...
    class FullPage {
        constructor(element, options) {
            ...
            this.bulletsWrapper = document.querySelectorAll(".fp-bullets");
        ...
        setActiveBullet(idButton) {
            ...
            const fpBullets = document.querySelectorAll(".fp-bullets");
            let bullets = Array.from(fpBullets);
            for (let index = 0; index < bullets.length; index++) {
                let bulletOnce = bullets[index].children;
                for (let i = 0; i < bulletOnce.length; i++) {
                    let bullet = bulletOnce[i];
                    if (idButton === i) bullet.classList.add(this.options.bulletActiveClass); else bullet.classList.remove(this.options.bulletActiveClass);
                }
            }
        }
        touchDown(e) {
        ...
        setBullets() {
            ...
            if (this.bulletsWrapper) {
                ...
                for (let index = 0; index < this.sections.length; index++) {
                    const section = this.sections[index];
                    section.setAttribute("data-fp-id", index);
                    nameSection = section.dataset.name;
                    arrNameSection.push(nameSection);
                }
                for (let index = 0; index < arrNameSection.length; index++) {
                    const link = document.createElement("a");
                    link.classList.add(this.options.bulletClass);
                    link.classList.add("menu__link");
                    link.innerHTML = arrNameSection[index];
                    fpBullets[0].append(link);
                }
                for (let index = 0; index < arrNameSection.length; index++) {
                    const link = document.createElement("a");
                    link.classList.add(this.options.bulletClass);
                    link.classList.add("decor-header__link");
                    link.innerHTML = arrNameSection[index];
                    fpBullets[1].append(link);
                }
            }
        }
        setZIndex() {
    ...
    if (window.innerWidth > 767.98 && window.innerHeight > 565 && document.querySelector("[data-fp]")) modules_flsModules.fullpage = new FullPage(document.querySelector("[data-fp]"), "");
    ...
    function playMusic() {
        const volume = document.querySelector("._icon-volume");
        const volumeOff = document.querySelector("._icon-volume-off");
    ...
    function showAllHelicopterList() {
        const showBtn = document.querySelector(".button-all");
        const showAll = document.querySelector(".two__all");
        const twoSmallContainer = document.querySelector(".two__small-container");
        const btnBack = document.querySelector(".all-two__back");
        showBtn.addEventListener("click", (() => {
            showAll.classList.add("active");
            twoSmallContainer.classList.add("active");
        }));
    ...
    if (window.innerWidth < 767.98) pageNavigation();
    headerScroll();
    preloader();
    playMusic();
    showAllHelicopterList();
})();